...
// https://www.npmjs.com/package/@snek/source-map-support
...
class BasicSourceMapConsumer {
  constructor(sourceMap, sourceMapURL) {
    ...
    this.sourceName = this._getSourceName(sourceRoot, sourceMapURL);
  ...
  /**
   * Get the original source name for the map the consumer
   * is being used on.
   * @param   {string} url  sourceMapURL passed to constructor
   * @param   {string} root original map.sourceroot
   * @returns {string|null} original file name associated with line.
   */
  _getSourceName(root, url) {
    try {
      const sourceArray = this._sources.toArray();

      return util.computeSourceURL(
        root,
        // for contrast's use case we're only ever passing
        // in one source file at a time (with a new consumer)
        sourceArray[0],
        url
      );
    } catch (e) {
      // this is the default failure condition for the original library.
      return null;
    }
  }
  ...
  originalPositionFor(args) {
    ...
    if (mapping) {
      if (mapping.generatedLine === needle.generatedLine) {
        ...
        if (source === null) {
          // in the event of our source map reading,
          // we want to grab the cached "top" source map
